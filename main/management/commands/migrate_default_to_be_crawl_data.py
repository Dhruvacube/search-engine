...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(
                settings.BASE_DIR /
                os.path.join("main", "management", "commands", "data",
                             "dump_data_new.csv"),
                "r",
                newline="",
        ) as f:
        ...
        for i in sites_url_data:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f'Problem in migration of "{i}": {e}'))
        self.stdout.write(
            self.style.NOTICE(f"{len(sites_url_data)} url(s) migrated"))
        self.stdout.write(
            "Now run `python manage.py crawl_to_be_crawled` command")
