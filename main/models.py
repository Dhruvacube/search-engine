...
class CrawledWebPages(models.Model):
    url = models.CharField(max_length=500,
                           help_text=_("Only domain name"),
                           unique=True)
    title = models.CharField(max_length=500,
                             help_text=_("The Title"),
                             blank=True,
                             null=True)
    sitemap_filepath = models.SlugField(
        max_length=250,
        help_text=_("The sitemap.xml file path"),
        null=True,
        blank=True)
    robot_txt_filepath = models.SlugField(
        max_length=250,
        help_text=_("The robot.txt file path"),
        null=True,
        blank=True)
    ...
    ip_address = models.GenericIPAddressField(protocol="both",
                                              ...
                                              null=True,
                                              blank=True)
    http_status = models.IntegerField(default=StatusCodes.OK,
                                      choices=StatusCodes.choices)
    scan_internal_links = models.BooleanField(default=True)

    keywords_meta_tags = ArrayField(models.CharField(max_length=500),
                                    null=True,
                                    blank=True,
                                    default=list)
    keywords_in_site = ArrayField(models.CharField(max_length=500),
                                  null=True,
                                  blank=True,
                                  default=list)

    stripped_request_body = models.TextField(
        null=True,
        blank=True,
        help_text=_("Mainly the description to display"))
...
class ToBeCrawledWebPages(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["url", "http_status"],
                name="url_and_http_status_idx",
                include=["last_crawled"],
            ),
        ]
